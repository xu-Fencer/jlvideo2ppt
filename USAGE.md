...
**Q: 支持哪些视频格式？**
A: 支持 MP4、AVI、MOV、MKV、WMV、FLV、WebM 等格式。确保已安装 ffmpeg。

**Q: 可以处理多长的视频？**
A: 理论上无限制，但建议单个视频不超过 2 小时。长视频可能需要更多时间和内存。

**Q: 页码识别支持哪些语言？**
A: 默认支持阿拉伯数字。如需识别中文页码（如"第5页"），需要训练自定义 Tesseract 模型。

**Q: 如何获得最佳检测效果？**
A: 使用高质量预设，调整 ROI 到页码位置，确保视频质量良好。

**Q: 可以自定义输出文件名格式吗？**
A: 目前支持三种命名策略。如需自定义，可以修改 `exporter.py` 中的 `_generate_filename` 方法。

**Q: 如何备份和恢复预设？**
A: 预设存储在 OUTPUT/presets 目录，可以直接复制该目录进行备份。

**Q: 可以在服务器上运行吗？**
A: 可以。使用 `--gui` 参数启动，然后通过浏览器访问。可以配置端口和访问控制。
